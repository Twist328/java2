...
class Calculator3{
   ...
   int getNum(){
        int num;
        try {
            num=Integer.parseInt(getSimbol());
        } catch (NumberFormatException e) {
            throw new NumberFormatException("Ошибочное значение числа");
        }
       return num;
   }
   boolean hasNext(){
   ...
   int bracketCalc(){
        String symbol=checkSimbol();
        if (symbol.equals("(")){
            getSimbol();
            int res=addSubCalc();
            String symbolNext=getSimbol();
            if (symbolNext.equals(")")) {
                return res;
            }else {
                try {
